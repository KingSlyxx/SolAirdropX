// server.js (BOG ინტეგრაციის გარეშე)
...
app.get('/', (req, res) => {
    res.json({
        message: 'LXRYTO Server is running (No BOG Payment Integration)',
        version: '1.0',
        endpoints: {
...
// --- 2. შეკვეთის გაგზავნა (გადახდის ინიციალიზაციის გარეშე) ---
...
📦 **გაყიდვების რაოდენობა:** ${data.totalSales}
    ...
    adminBot.onText(/\/recent\s?(\d*)/, async (msg, match) => {
        ...
        try {
            ...
            if (sales.length === 0) {
                return adminBot.sendMessage(chatId, 'ბოლო შეკვეთები ვერ მოიძებნა.');
            }
...
${salesList}
            ...
            adminBot.sendMessage(chatId, '❌ შეცდომა ბოლო შეკვეთების მიღებისას.');
        }
    });
